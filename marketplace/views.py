...
def add_to_cart(request, food_id):
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':  # request.is_ajax() is deprecated
            # check if fooditem exists
            try:
                fooditem = FoodItem.objects.get(id=food_id)
                # Check if the user have already added food to the cart
                try:
                    chkCart = Cart.objects.get(user=request.user, fooditem=fooditem)
                    # Increase the cart qtty
                    chkCart.quantity += 1
                    chkCart.save()
    ...
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':  # request.is_ajax() is deprecated
            ...
            try:
                ...
                try:
                    chkCart = Cart.objects.get(user=request.user, fooditem=fooditem)
                    # Decrease the cart qtty
                    if chkCart.quantity > 1:  # changed from >= because, cart wanst correctly deleted
                        chkCart.quantity -= 1
                        chkCart.save()
                    else:
                        chkCart.delete()
